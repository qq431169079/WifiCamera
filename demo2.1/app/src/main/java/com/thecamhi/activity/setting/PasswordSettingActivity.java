...
public class PasswordSettingActivity extends HiActivity implements ICameraIOSessionCallback, TextWatcher {
	...
	private void initView() {
		...
		edt_current_password.setFilters(new InputFilter[] { new InputFilter.LengthFilter(31), new SpcialCharFilter(PasswordSettingActivity.this), new EmojiFilter() });
	...
	public void updatePassword() {
		...
		if (newPassword.contains("~") || newPassword.contains("=") || newPassword.contains("&") || newPassword.contains("\\") || newPassword.contains("\"") || newPassword.contains(" ")
		...
		// || ("admin".equalsIgnoreCase(newPassword) &&
		// "admin".equalsIgnoreCase(confirmPwd))) {
		// HiToast.showToast(PasswordSettingActivity.this,
		// getString(R.string.tip_not_empty_admin));
		// return;
		// }
		if ((TextUtils.isEmpty(newPassword) && TextUtils.isEmpty(confirmPwd))) {
		...
		if (newPassword.getBytes().length >= 32 || confirmPwd.getBytes().length >= 32) {
