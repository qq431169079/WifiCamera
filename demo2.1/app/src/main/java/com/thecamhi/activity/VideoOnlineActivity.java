...
public class VideoOnlineActivity extends HiActivity implements ICameraIOSessionCallback, OnItemClickListener, ICameraDownloadCallback {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		HiTools.cameraWhether(VideoOnlineActivity.this, mCamera);
		initView();
		searchVideo();
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
		...
		if (resultCode == RESULT_OK) {
			...
			// list_video_online.removeAllViews();
			if (list_video_online.getHeaderViewsCount() == 0) {
				list_video_online.addHeaderView(searchTimeView);
			}
	...
	private Handler handler = new Handler() {
		...
		private void handDownLoad(Message msg) {
			...
			case DOWNLOAD_STATE_ERROR_PATH:
				showAlert(getResources().getString(R.string.tips_wifi_connect_failed), new DialogInterface.OnClickListener() {
		...
		mDownDialog.findViewById(R.id.btn_downlinad_video_online).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (HiTools.checkPermission(VideoOnlineActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
					...
					ActivityCompat.requestPermissions(VideoOnlineActivity.this, new String[] { Manifest.permission.WRITE_EXTERNAL_STORAGE }, 0);
				}
			}
		});
		// 3.取消
